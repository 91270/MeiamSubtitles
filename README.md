...
- **正式版**：版本号末位为 `0`，例如 `1.0.18.0`。正式版会发布到 GitHub Releases，并同步到 Jellyfin 稳定插件仓库。
- **测试版（Preview）**：GitHub 只保留一个滚动更新的“测试版（滚动更新）”，固定使用 `preview` 标签和 `Emby_preview.zip`、`Jellyfin_preview.zip` 两个附件。GitHub 中仍标记为 Pre-release，以区别于稳定发布。
- 每次更新测试版时，插件内部版本末位继续递增，例如 `1.0.17.1`、`1.0.17.2`，用于识别用户实际安装的测试批次；旧测试包会被新包替换。
- 测试版不会写入稳定插件仓库，测试用户需要从 GitHub 的 `preview` Release 手动下载安装。
- 正式版版本号必须高于同一轮全部测试版。例如 `1.0.17.1`、`1.0.17.2` 验证完成后发布 `1.0.18.0`。
- 正式版发布后，滚动测试 Release 会转为草稿保留；开始下一轮测试时再更新并重新公开。
